...
export default function ReportDetailsUpdateForm({
    reportId
}: {
    reportId: Id<'reports'>
}) {
    const report = useQuery(api.reports.getReportById, { reportId });
    const findingsQuery = useQuery(api.reports.getFindingsByReport, { reportId });
    ...
    type UpdateReportArgument = FunctionArgs<typeof api.reports.updateReport>
    type FormData = Omit<UpdateReportArgument, 'reportId'>

    const [isLoading, setIsLoading] = useState(false);
    const [isUploading, setIsUploading] = useState(false);
    const [findingsState, setFindingsState] = useState<NonNullable<UpdateReportArgument['findings']>>([]);
    ...
    // Pre-populate form when report data is loaded
    useEffect(() => {
        if (report) {
            form.reset({
                draft: report.draft ?? false,
    ...
    // Load findings when they're available
    useEffect(() => {
        if (findings && findings.length > 0) {
            setFindingsState(findings.map(finding => ({
                findingNumber: finding.findingNumber,
                findingType: finding.findingType,
                description: finding.description,
                impactRating: finding.impactRating,
                likelihoodRating: finding.likelihoodRating,
    ...
    async function onSubmit(data: FormData) {
        ...
        try {
            const updatedReportId = await updateReport({
                reportId,
                draft: data.draft,
            ...
            >
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Project and reporting period information (read-only)</CardDescription>
                    </CardHeader>
                    <CardContent className='grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-6'>
                        {/* Project Display (Read-only) */}
                        <div className='sm:col-span-2'>
                            <Label>Project</Label>
                            <div className='mt-2 h-[45px] p-3 border rounded-md bg-muted/50 flex items-center'>
                                <p className='text-sm font-medium truncate'>{project?.projectName ?? 'Loading...'}</p>
                            </div>
                        </div>
                        ...
                        <div className='sm:col-span-2'>
                            <Label>Month</Label>
                            <div className='mt-2 h-[45px] p-3 border rounded-md bg-muted/50 flex items-center'>
                                <p className='text-sm font-medium'>{months.find(m => m.value === report.month)?.label ?? report.month}</p>
                            </div>
                        </div>
                                        ...
                                        <Input
                                            ...
                                            onChange={async (event) => {
                                                ...
                                                if (file) {
                                                    ...
                                                    form.setValue('attachmentId', report.attachmentId);
                                                }
                                            }}
                                        />
                                    </FormControl>
                                    <FormDescription>
                                        {field.value ? 'New file will replace the existing attachment' : 'No attachment uploaded'}
                                    </FormDescription>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardAction>
                            <ReportFindingDialog
                                addFinding={(finding) => setFindingsState(prev => [...prev, finding])}
                            />
                        </CardAction>
                    </CardHeader>
                                        ...
                                        <div className='flex flex-col gap-1'>
                                            <div className='flex flex-row gap-2 justify-between items-center'>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => setFindingsState(prev => prev.filter((_, i) => i !== index))}
                                                >
